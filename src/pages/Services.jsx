...
import { services, process, whyChoose } from '../data/content';
  ...
  return (
    <div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16 lg:pb-24">
        <Stagger className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 lg:gap-6" staggerDelay={0.07}>
          {services.map((s) => (
            ...
            {process.map((p, i) => (
              <StaggerItem key={p.step}>
                <div className="bg-white rounded-2xl border border-gray-100 p-4 sm:p-5 h-full shadow-card hover:shadow-elev transition-shadow">
                  <span className="text-xl sm:text-2xl font-extrabold text-primary-200">{p.step}</span>
